...
--end
local DinoWindow = Dino:CreateWindow("BountyFarm Update4", "Seen")
...
local DinoDropdown = DinoElement:CreateDropdown("Select Player", "rbxassetid://7831282709", players, function(list)
...
local DinoDropdown = DinoElement:CreateDropdown("Select Gun", "rbxassetid://7831282709", gunl, function(list2)
     _G.GUNSELECT = list2
end)
...
TP(game.Players[_G.PLAYERSELECT].Character.HumanoidRootPart.Position, game.Players[_G.PLAYERSELECT].Character.HumanoidRootPart.CFrame * CFrame.new(0,60,5))
    end
end
...
DinoElement:CreateToggle("Kill [GunHopPC]", "rbxassetid://7839505809", function(value)
    _G.KillGunHop = value
    AimbotLock = value
    _G.Shoot = value
...
DinoElement:CreateToggle("Kill [Combat]", "rbxassetid://7839505809", function(value)
    _G.KillCombat = value
    if _G.KillCombat == true then
    while _G.KillCombat do wait()
function TP(P1,P2)
    local Distance = (P1 - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
    if Distance < 1000 then
        Speed = 50
...
TP(game.Players[_G.PLAYERSELECT].Character.HumanoidRootPart.Position, game.Players[_G.PLAYERSELECT].Character.HumanoidRootPart.CFrame * CFrame.new(0,20,5))
...
if _G.KillCombat == false then
    game.Players[_G.PLAYERSELECT].Character.HumanoidRootPart.Size = Vector3.new(1,1,1)
    game.Players[_G.PLAYERSELECT].Character.HumanoidRootPart.Transparency = 1
end
end)
DinoElement:CreateToggle("Kill [CombatHopPC]", "rbxassetid://7839505809", function(value)
    _G.KillCombatHop = value
    if _G.KillCombatHop == true then
    while _G.KillCombatHop do wait()
function TP(P1,P2)
    local Distance = (P1 - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
    if Distance < 1000 then
        Speed = 50
...
DinoElement:CreateToggle("KillBetaTest [Combat]", "rbxassetid://7839505809", function(value)
    _G.bounty = value
		while _G.bounty do wait()
			for i,v in pairs(game:GetService("Workspace").Characters:GetChildren()) do
				if v.Name == _G.PLAYERSELECT then
					repeat wait()
...
TP(game.Players[_G.PLAYERSELECT].Character.HumanoidRootPart.Position, game.Players[_G.PLAYERSELECT].Character.HumanoidRootPart.CFrame * CFrame.new(0,10,5))
...
DinoElement:CreateButton("Hop[PC]", "rbxassetid://7839505809", function(value)
local PlaceID = game.PlaceId
        local AllIDs = {}
        local foundAnything = ""
        local actualHour = os.date("!*t").hour
...
local DinoPage = DinoWindow:NewPage("Misc")
local miscpage = DinoPage:NewSection("Misc")

miscpage:CreateSlider("Speed[แนะนำใกล้ปรับ2ไกลปรับ45]", "rbxassetid://7839722369", 1, 500, function(value)
    spd = value
end)
miscpage:CreateToggle("MobAuraHaveBug[Beta]", "rbxassetid://7839505809", function(value)
_G.AutoDun = value
if _G.AutoDun == true then
while _G.AutoDun do wait(0.1)
game:GetService'VirtualUser':Button1Down(Vector2.new(1280, 672))
for i, a in pairs(game.Workspace.Enemies:GetChildren()) do
    a.HumanoidRootPart.Size = Vector3.new(10,10,10)
    a.HumanoidRootPart.Transparency = 0.75
    a.Humanoid.WalkSpeed = 0
end
for i,we in pairs(game.Workspace.Enemies:GetDescendants()) do
                if we.Name == "Head" then
local tws = game:GetService('TweenService')
local info = TweenInfo.new(spd)
local tw = tws:Create(game.Players.LocalPlayer.Character.HumanoidRootPart, info, {CFrame = we.CFrame})
tw:Play()
local tws = game:GetService('TweenService')
local info = TweenInfo.new(spd)
local tw = tws:Create(game.Players.LocalPlayer.Character.HumanoidRootPart, info, {CFrame = we.CFrame})
...
if _G.AutoDun == false then
for i, m in pairs(game.Workspace.Enemies:GetChildren()) do
    m.HumanoidRootPart.Size = Vector3.new(2,2,2)
    m.HumanoidRootPart.Transparency = 1
end
end
end)
miscpage:CreateToggle("Fastattack[Lag,Ping]", "rbxassetid://7839505809", function(value)
    _G.FastAttk = value
...
miscpage:CreateToggle("KillAllBeta", "rbxassetid://7839505809", function(value)
_G.ka = value
if _G.ka == true then
while _G.ka do wait()
for i,pl in pairs(game.Workspace.Characters:GetDescendants()) do
                if pl.Name == "HumanoidRootPart" then
function TP(P1,P2)
    local Distance = (P1 - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
    if Distance < 1000 then
        Speed = 150
...
page:CreateLabel("----------------------------")
page:CreateLabel("Developper คนชนเทพ#7434")
page:CreateLabel("UiCredit Sanda#9197")
page:CreateLabel("[+] = Add")
page:CreateLabel("[-] = Remove")
page:CreateLabel("[?] = FixBug")
page:CreateLabel("----------------------------")
page:CreateLabel("Update 4")
page:CreateLabel("[-] TweenSpeed")
page:CreateLabel("[+] AddTweenKill")
page:CreateLabel("[?] FixTweennotSmooth")
return Dino
